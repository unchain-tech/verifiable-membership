...
import { IssuerFactory, CertIssuerProfileFactory, CertificateFactory } from '@utility';
...
test('check', () => {
  ...
  const certificate = createUnsignedCertificate(issuer.rawDocument, holder)



  let certificate = new Certificate(certificate.rawDocument);
